...
//----------------------------------------------------------------------------//
// Constructor.
Policy<AmbientLight>::Policy(Toolset &toolset,
                             UnitSelector& unit_selector,
                             Policies& engine_policies) : tools(toolset)
{}

// Initializes a provided light unit.
void Policy<AmbientLight>::Initialize(AmbientLight& light)
{
	ut::Array<AmbientLight::FrameData> frames;
	for (ut::uint32 i = 0; i < tools.config.frames_in_flight; i++)
	{
		...
		AmbientLight::FrameData frame_data{ uniform_buffer.Move() };
